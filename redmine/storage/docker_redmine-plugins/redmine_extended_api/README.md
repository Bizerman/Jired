...
- Add an imported journal entry to an existing issue, keep the original editor and timestamp, and avoid triggering notifications:
  ...
  curl -X PATCH \
    -H "Content-Type: application/json" \
    -H "X-Redmine-API-Key: <token>" \
    -d '{"issue":{"notes":"Imported worklog from jired","journal":{"user_id":7,"updated_by_id":7,"created_on":"2023-12-01T11:00:00Z","updated_on":"2023-12-01T12:00:00Z"}},"notify":false}' \
  ...
  ```json
  {
    ...
    "journal": {
      "id": 456,
      "issue_id": 123,
      "notes": "Imported worklog from jired",
...
- Upload an attachment while preserving the original uploader and timestamp (admin users only):
  ...
  curl -X PATCH \
    ...
    -d '{
    "issue": {
    "notes": "Imported worklog from jired: admin, attachment included",
    "journal": {
    "user_id": 9,
    "updated_by_id": 9,
    "created_on": "2023-12-01T11:00:00Z",
